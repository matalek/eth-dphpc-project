...
public:
    ...
    void merge(shared_ptr<ConvexHullRepresentation> hull) {
        shared_ptr<VectorConvexHullRepresentation> other_hull = std::static_pointer_cast<VectorConvexHullRepresentation>(
                hull);
    ...
    void merge_lower_hull(VectorConvexHullRepresentation &other_hull) {
        ...
        vector<POINT *> merged_vector;
        merged_vector.reserve(low_tangent.first + 1 + other_hull.get_hull()->size() - low_tangent.second); // preallocate memory
        merged_vector.insert(merged_vector.end(), hull->begin(), hull->begin() + low_tangent.first + 1);
        merged_vector.insert(merged_vector.end(), other_hull.get_hull()->begin() + low_tangent.second, other_hull.get_hull()->end());
        hull = shared_ptr<vector<POINT *> >(new vector<POINT *>(merged_vector));
    }
    ...
    void merge_upper_hull(VectorConvexHullRepresentation &other_hull) {
        ...
        vector<POINT *> merged_vector;
        merged_vector.reserve(upper_tangent.first + 1 + upper_tangent.second + 1); // preallocate memory
        merged_vector.insert(merged_vector.end(), other_hull.get_hull()->begin(), other_hull.get_hull()->begin() + upper_tangent.second + 1);
        merged_vector.insert(merged_vector.end(), hull->begin() + upper_tangent.first, hull.get()->end());
        hull = shared_ptr<vector<POINT *> >(new vector<POINT *>(merged_vector));
    }
