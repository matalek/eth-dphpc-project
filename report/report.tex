...
We want to investigate parallel algorithms for sorted input, implement them and compare them. Our implementation is based on already known algorithms. We want to explore performance of each algorithm using up to 256 processors and different shapes for input data, then compare results with theoretical boundary related to algorithm and shape.
...
First algorithm we decided to implement is Naive Parallel algorithm inspired by Generic Divide-and-Conquer Solution to the Convex Hull Problem \cite{NaiveParallel}

Second is Simple Parallel Convex Hulls Algorithm \cite{SimpleParallel}  Simple Parallel algorithm has published results even for different shapes, but only up to 8 processors.   
 
Third is Hull Tree Algorithm \cite{HullTree}, both Neither  implementation of Hull Tree algorithm nor its performance was published.
 
Every algorithm is designed to solve convex hull problem for sorted input.
...
is a common tangent between Upper (Lower) hulls. 
