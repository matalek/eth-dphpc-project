...
\begin{abstract}
The problem of computing the 2-D convex hull of a set of points finds practical applications in many different fields, like pattern recognition, image processing, statistics and many others.
...
We chose 3 algorithms that meet our requirements: work with sorted input and use shared memory. We used a sufficient number of processors for the first two algorithms to reach their optimality theoretical boundary. 
...
For big hulls it's better to replace the linear iterative search in step 2 with a binary search. Then we can find a candidate from possible range in a logarithmic number of steps.       
...
For simplicity in all the algorithms we will assume that $k$ is a power of 2. 
...
Each of the algorithms presents different ways of merging partial convex hulls.
