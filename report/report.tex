...
\BEGIN \\ %
  left\_point := A.rightmost\_point;
  right\_point := B.leftmost\_point;
\WHILE !(is\_tangent(A,A\_point, B\_point) \AND 
\STATE{ (is\_tangent(B,A\_point, B\_point))} \DO
	  \WHILE !is\_tangent(A,A\_point, B\_point) \DO
		A\_point := A\_point.counter\_clockwise()	    
	  \OD 
	  \WHILE !is\_tangent(B,A\_point, B\_point) \DO
		A\_point := A\_point.\_clockwise()	    
	  \OD  
 \OD	
\END
\end{program}
