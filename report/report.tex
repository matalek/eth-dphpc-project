...
We want to investigate parallel algorithms for sorted input in PRAM (Parallel Random Access Machine), implement them and compare them.
It is important to distinguish between convex hull problems with sorted or unsorted input, because they have different sequential complexity: $O(n)$ and $O(n\log n)$ respectively.

Our implementations are based on already existing algorithms and we want to explore the performance of these algorithms using up to 256 processors and different shapes for input data.
