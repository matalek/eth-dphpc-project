...
For square and disk we obtained very good results when changing the number of working threads: for 2, 4 and 8 threads we even obtained superlinear speedup with the HullTree algorithm.
...
The other algorithms still have decent speedups but not as good as the square.
The more memory accesses needed and the parallel overhead are now more present.
\begin{figure}[!ht]\centering
  \includegraphics[scale=0.33]{./plots/speedup_xeon_square_10000000.eps}
  \caption{Speedup of the algorithms with the given number of threads for an input set of 10M points in a square\label{Threads time}}
\end{figure}
