...
\begin{abstract}
The problem of computing the 2D convex hull of a set of points finds practical applications in many different fields, like pattern recognition, image processing, statistics and many others.
The main contribution of this paper is to review some available parallel algorithms for computing in the PRAM model the convex hull of a sorted set of points in the plane.
...
In this paper, we study the two-dimensional convex hull problem, fundamental in computation geometry.

We want to investigate parallel algorithms for sorted input in PRAM (Parallel Random Access Machine), implement them and compare them. It is important to distinguish between a convex hull problem with or without sorted input, because they have different sequential complexity: $O(n)$ and $O(n\log n)$ respectively.

Our implementations are based on already existing algorithms.
We want to explore the performance of the algorithms using up to 256 processors and different shapes for input data.
We are interested in real performance including hidden constants and overhead caused by different data structures and structure of computations.
...
We chose 3 algorithms that meet our requirements: work with sorted input and use shared memory.
The number of processors that we used met the boundary for the second and third algorithms to be optimal. 

The first algorithm we decided to implement is inspired by \cite{NaiveParallel}, further referred to as \textit{Naive} for the naivety of the concept behind it.
It works in PRAM EREW model.

The second one is derived from \cite{SimpleParallel}, further referred to as \textit{Simple} according to the title of the paper we derived it from.
It works in PRAM CREW model.
 
The third one appears in \cite{HullTree}, further referred to as \textit{HullTree} in relation to the data structure it is based on.
It works in PRAM CREW model.
     
\section{Background: Convex hull problem}\label{sec:background}

\mypar{Convex polygon} A convex polygon is defined as a polygon with all its interior angles less than $\pi$.\\
\mypar{Convex hull}
...
For simplicity we assume that $x$-coordinates of the input set are unique.\\
\mypar{Tangent} 
...
The purpose of this section is to review the common tangent line algorithm and present parallel algorithms for computing a convex hull of a sorted set of points.

\mypar{Common tangent line algorithm}
Let us consider convex hulls $A$ and $B$ so that every point from $A$ has $x$-coordinate lower than each point from $B$. 
These convex hulls are disjoint and 2 outer common tangents exist. 
The idea of the algorithm for computing them is to alternate searching for tangent endpoints between the two polygons, until both ends of the line segment simultaneously satisfy the tangency condition. 

Firstly, we find the upper tangent:
	\begin{enumerate}
		\item Start with the rightmost point of $A$ and the leftmost point of $B$ and consider a line passing through them as a candidate for the upper tangent line.
...
For convex hulls consisting of big number of points it is better to replace the linear iterative search in step 2 with a binary search, improving time complexity to $O(\log n*\log m)$.
Then we can find a candidate from possible range in a logarithmic number of steps.       
...
Computing these intervals can be done by computing the common tangent line of each pair $C_i$ and $C_j$, using the same approach as used in {\it Naive} algorithm. 
...
The last step consist of moving all taken points to one resulting array.
...
%TODO
The platform we tested our algorithms with mounts Intel\textsuperscript{\textregistered} Xeon Phi processors. They are based on the Intel\textsuperscript{\textregistered} Many Integrated Core (MIC) architecture.
...
{\bf Circle}: Randomly generated angles $\theta_n$ and created the points ($\lfloor R \cos(\theta_n)\rfloor$, $\lfloor R \sin(\theta_n)\rfloor$).
...
We show performance with 8 processing threads to represent the trends of the algorithms, which is generally the same (with different speedups) for the tested number of threads.
...
Clearly the higher the number of points, the higher the efficiency obtained by splitting the point set among threads.
...
For circle [Plot \ref{Threads speedup circle}] the situation is again more complicated, since the \textit{HullTree} algorithm has a very bad performance, but the trend is very similar with a fast increase in efficiency followed by a close-to-steady state and a decrease in efficiency for a high number of threads.
