...
\usepackage{spconf,amsmath,amssymb,graphicx,amsthm,newlfont,program}
...
Then we know that points $p_{i+1}, p_{i + 2}, ..., p_{n - 1}$ and $q_{j + 1}, q_{j + 2}, ..., q_{m - 1}$ are not gonna be in the resulting convex hull.
Therefore, we have:
...
Example scheme for 4 processors is presented on figure \ref{naive-parallel-scheme}.
